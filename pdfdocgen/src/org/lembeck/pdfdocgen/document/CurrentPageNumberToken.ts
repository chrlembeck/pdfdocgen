...
export class CurrentPageNumberToken implements Token {
  ...
  addToPageLayout(pageLayout: PageLayout, cursorX: number, cursorY: number, widthMM: number, paragraph: Paragraph, state: LayoutState): void {
    const layoutedText = new LayoutedText(cursorX, cursorY, String(pageLayout.pageNumber), 'left', widthMM, this._font, this._fontSize, this._fontColor || paragraph.color);
  ...
  splitToken(pdf: jsPDF, remainingMM: number, state: LayoutState, pageLayout: PageLayout): SplitToken {
    ...
    const width = pdf.getTextWidth(String(pageLayout.pageNumber));
